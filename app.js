...
async function initTenderPage() {
  ...
  function renderView(rows) {
    ...
    if (gaugesEl) {
      if (!ongoingForGauges.length) {
        ...
        gaugesEl.innerHTML=ongoingForGauges.map((r,i)=>{
          ...
          return `<div style="text-align:center;min-width:200px;${isExpired?'animation:expiredPulse 1.2s ease-in-out infinite;':''}" title="${r['Tender No']||''} · ${poleType} · Limit: ${limitDays}d">
            <svg viewBox="0 0 180 105" width="200" height="116" style="overflow:visible;display:block;margin:0 auto">
              <defs>
                <filter id="glow-${i}"><feGaussianBlur stdDeviation="5" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>
              </defs>
              <!-- Single glowing arc — no background track -->
              <path d="${singleArc}" fill="none" stroke="${col}" stroke-width="14" stroke-linecap="round" filter="url(#glow-${i})"/>
              <!-- Needle stem -->
              <path d="${needleStem}" stroke="${col}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>
              <!-- Needle arrowhead -->
              <path d="${arrowHead}" fill="${col}" style="filter:drop-shadow(0 0 4px ${col})"/>
              <!-- Pivot -->
              <circle cx="${cx}" cy="${cy}" r="5" fill="${col}" stroke="rgba(0,0,0,.6)" stroke-width="1.5"/>
              <!-- Value -->
              <text x="${cx}" y="${cy-16}" text-anchor="middle" font-size="18" font-weight="800" fill="${col}" font-family="monospace">+${Math.round(ageDays)}d</text>
            </svg>
            <div style="font-size:.70rem;font-weight:700;color:${col};margin-top:.1rem;letter-spacing:.5px">△ +${Math.round(ageDays)}d</div>
            <div style="font-size:.62rem;color:rgba(255,255,255,.55);margin-top:.18rem;font-weight:600">${r['Tender No']||'—'}</div>
            <div style="font-size:.58rem;color:rgba(255,255,255,.35);margin-top:.1rem">${poleType||'—'} · ${r['Site Name']||'—'}</div>
          </div>`;
        }).join('');
    ...
    function buildDelayMeter(delayVal, rowIdx) {
      ...
      const needleLen=rad-10, arrowSize=7;
      ...
      return `<div class="inline-meter-wrap${isDelayed?' m-delayed':''}" style="text-align:center;min-width:200px;">
        <svg viewBox="0 0 180 105" width="200" height="116" style="overflow:visible;display:block;margin:0 auto">
          <defs><filter id="${filtId}"><feGaussianBlur stdDeviation="5" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter></defs>
          <path d="${singleArc}" fill="none" stroke="${col}" stroke-width="14" stroke-linecap="round" filter="url(#${filtId})"/>
          <path d="${needleStem}" stroke="${col}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>
          <path d="${arrowHead}" fill="${col}" style="filter:drop-shadow(0 0 4px ${col})"/>
          <circle cx="${cx}" cy="${cy}" r="5" fill="${col}" stroke="rgba(0,0,0,.6)" stroke-width="1.5"/>
          <text x="${cx}" y="${cy-16}" text-anchor="middle" font-size="18" font-weight="800" fill="${col}" font-family="monospace">${valTxt}</text>
        </svg>
        <div style="font-size:.70rem;font-weight:700;color:${lblCol};margin-top:.1rem;letter-spacing:.5px">${lblTxt}</div>
      </div>`;
    }
